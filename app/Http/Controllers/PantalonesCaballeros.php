...
class PantalonesCaballeros extends Controller{
            ...
            if(isset($_POST['Name'])){
                ...
                if($conn){
                    ...
                    if (mysqli_query($conn, $sql)) {
                        return view('pantalonesCaballeros')->with('nombre', $nombre);
                    }
            }
            return view('pantalonesCaballeros');
            }else{
            $conn = mysqli_connect("localhost", "root", "", "chein");
            $correo = $_POST['Email'];
            $password = $_POST['Password'];
            if($conn){
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {         
                        ...
                        return view('pantalonesCaballeros')->with('nombre', $row["name"])->with('email', $row["id"] )->with('productos', $productos);
                    }
