...
class VestidosGirls extends Controller{
    ...
    public function iniciar(){
        if(isset($_POST['Email']) && isset($_POST['Password'])){
            ...
            if(isset($_POST['Name'])){
                ...
                if($conn){
                    ...
                    if (mysqli_query($conn, $sql)) {
                        return view('vestidosGirls')->with('nombre', $nombre);
                    }
            }
            return view('vestidosGirls');
            }else{
            $conn = mysqli_connect("localhost", "root", "", "chein");
            $correo = $_POST['Email'];
            $password = $_POST['Password'];
            if($conn){
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {         
                        ...
                        return view('vestidosGirls')->with('nombre', $row["name"])->with('email', $row["id"] )->with('productos', $productos);
                    }
            ...
            $productos = buscarproductos();
            return view('vestidosGirls')->with('productos',$productos);        
        }
