...
class PlayerasCaballeros extends Controller{
...
/*     public function iniciar(){
    ...
    public function iniciar(){
        if(isset($_POST['Email']) && isset($_POST['Password'])){
            ...
            if(isset($_POST['Name'])){
                ...
                if($conn){
                    ...
                    if (mysqli_query($conn, $sql)) {
                        return view('playerasCaballeros')->with('nombre', $nombre);
                    }
            }
            return view('playerasCaballeros');
            }else{
            $conn = mysqli_connect("localhost", "root", "", "chein");
            $correo = $_POST['Email'];
            $password = $_POST['Password'];
            if($conn){
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {         
                        ...
                        return view('playerasCaballeros')->with('nombre', $row["name"])->with('email', $row["id"] )->with('productos', $productos);
                    }
            ...
            $productos = buscarproductos();
            return view('playerasCaballeros')->with('productos',$productos);        
        }
